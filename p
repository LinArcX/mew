  ...
  case $selected in
    "build(debug)")
      ...
      bear -- g++ -std=c++23 -g -pg -O0 -DDEBUG --coverage \
        src/*.cpp $(pkg-config --cflags --libs x11 xft fontconfig freetype2) -o build/debug/mew
    ...
    "run(debug)")
      ...
      ;;
    "gf2")
      gf2 -nx -ex "source breakpoints.gdb" build/debug/mew
      ;;
    "gf2 attach")
      ...
      gf2 -nx -ex "source breakpoints.gdb" -p "$(pidof mew)" >/dev/null 2>&1 & disown
      sleep 2
      wmctrl -r "gf2" -e 0,0,500,1820,550

      #gf2 -nx -ex "source breakpoints.gdb" -p $(pidof mew) &
      ...
      ;;
    "rr record")
      /usr/bin/rr record build/debug/mew
      ;;
    "gf2 --rr-replay")
    ...
    "build(tests)")
      ...
      cc -g -pg -O0 -DDEBUG -std=c11 --coverage \
        ...
        -lc -lcmocka -lmagic -o ./build/tests/mewt \
    ...
    "cppcheck")
      ...
      cppcheck-htmlreport --file=report/cppcheck.xml --title="mew" --report-dir=report --source-dir=.
      ;;
    "cppcheck(show)")
    ...
    "llvm-cov")
      ;;
    "valgrind(memcheck)")
      valgrind --tool=memcheck --leak-check=full --show-leak-kinds=all --track-origins=yes --xtree-leak=yes -s -v build/debug/mew
      ;;
    "callgrind")
      valgrind --tool=callgrind --dump-instr=yes --collect-jumps=yes -s -v build/debug/mew
      ;;
    "kcachegrind")
      ls callgrind.out.* cachegrind.out.* | fzf --header="kcachgrind: " | xargs kcachegrind
      ;;
    "cachegrind")
      valgrind --tool=cachegrind --cache-sim=yes --branch-sim=yes -s -v build/debug/mew
      ;;
    "helgrind")
      valgrind --tool=helgrind -s -v build/debug/mew
      ;;
    "massif")
      valgrind --tool=massif -s -v build/debug/mew
      ;;
    "ms_print")
      ls massif.out.* | fzf --header="ms_print: " | xargs ms_print
      ;;
    "drd")
      valgrind --tool=drd --trace-fork-join=yes --trace-mutex=yes --trace-semaphore=yes -s -v build/debug/mew
      ;;
    "dhat")
      valgrind --tool=dhat -s -v build/debug/mew
      ;;
    "dhat(cat)")
      ls dhat.out.* | fzf --header="dhat: " | xargs cat | less
      ;;
    "bbv")
      valgrind --tool=exp-bbv -s -v build/debug/mew
      ;;
    "bbv(cat)")
    ...
    "perf")
      ...
      ;;
    "uftrace record(test)")
      uftrace --srcline -a --symbols --time --no-libcall --symbols -F __clove_symint___UtilSuite* record build/debug/mew
      ;;
